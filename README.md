...
`dm_control cartpole swingup`에서 강화학습 전체 흐름을 처음부터 끝까지 구현하고, 짧은 학습과 충분한 학습의 차이를 보상과 GIF로 확인한 강화학습 포트폴리오.
...
그래서 복잡한 보상 튜닝이나 알고리즘 확장보다, 공식 `cartpole swingup` 과제를 기준으로 작지만 끝까지 다시 실행해 볼 수 있는 학습 실험 틀을 만드는 것을 목표로 했다.
...
- `25,000` 스텝 단기 학습과 `100,000` 스텝 전체 학습을 비교해 보상과 실제 동작을 확인했다.

## 결과

`25,000` 스텝에서는 정책이 스윙업 전략을 배우기 시작한 단계에 머물렀고, `100,000` 스텝에서는 보상 기준으로도 크게 향상됐다.  
다만 임의로 선정한 성공 보상 기준값 `400.0`으로 계산하면 여전히 `0%`여서, 성공 판단 기준값을 어떤 기준으로 설정 하느냐도 고민해볼 문제이다.

| 학습 조건 | 대표 보상 | 보조 지표 | 해석 |
| --- | --- | --- | --- |
| `25,000` 스텝 | 최종 평가 평균 보상 `105.23` | 학습 중 평균 보상 `56.1` | 스윙업은 시작했지만 안정적 도립 유지에는 아직 못 미침 |
| `100,000` 스텝 | 최종 평가 평균 보상 `358.10` | 최고 성능 모델 재평가 평균 보상 `358.53` | GIF 기준으로 거의 성공에 가까운 동작 |
...
| 평가 | 저장된 모델의 평균 보상과 성공률 계산 | [evaluate.py](evaluate.py) |
